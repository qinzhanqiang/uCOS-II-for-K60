...
* 
*
* INT16U  OSSemAccept (OS_EVENT *pevent);                       //当信号量无效时准许任务不进入等待状态而是继续运行
* 
* OS_EVENT  *OSSemCreate (INT16U cnt);                          //创建一个信号量
* 
* void  OSSemPend (OS_EVENT  *pevent,                           //请求信号量
*                 INT32U     timeout,
*                 INT8U     *perr);                             //任务调用该函数请求信号量
* 
* INT8U  OSSemPost (OS_EVENT *pevent);                          //发送（释放）信号量
* 
...
*                  OS_SEM_DATA  *p_sem_data);                   //查询信号量的当前状态
* 
...
INT16U  OSSemAccept (OS_EVENT *pevent)                  //检查信号量
{
    INT16U     cnt;
